...
from torchblocks.losses.cross_entropy import SoftTargetCrossEntropy
from torchblocks.losses.label_smoothing import LabelSmoothingCE
from torchblocks.losses.MutiLabel import multilabel_crossentropy
...
class TrainerBase:
    ...
    keys_to_ignore_on_result_save = ['entities', 'message', "pub_time", "title", "content"]  # eval和predict结果不存储的变量
    ...
    def build_model_param_optimizer(self, model):
        ...
        if hasattr(model, self.opts.base_model_name) and self.opts.other_learning_rate != 0.0:
            ...
            self.logger.info(msg)
            base_model_param = list(model.convs.named_parameters())
    ...
    def build_eval_dataloader(self, dev_data):
        ...
        if isinstance(dev_data, DataLoader):
            ...
            # sampler = DistributedSampler(dev_data, shuffle=True)
    ...
    def build_test_dataloader(self, test_data):
        ...
        if isinstance(test_data, DataLoader):
            ...
            # sampler = DistributedSampler(test_data, shuffle=True)
    ...
    def build_batch_inputs(self, batch):
        ...
        batch_tensor = batch['father'].batch.to(self.device)
        inputs = {'label': batch.y, "dict_": batch.dict_, "old_dict": batch.x_dict, "edge_index_dict": batch.edge_index_dict, "batch": batch_tensor}
        batch['label'] = batch.y
        return inputs
    ...
    def train_forward(self, batch):
        ...
        if self.opts.loss_type is not None:
            if self.opts.loss_type == 'MutiChoice':
                ...
                loss_mine = multilabel_crossentropy(outputs, batch["label"].to(self.device))
                outputs_dict = {}
                outputs_dict['logits'] = outputs
                outputs_dict['loss'] = loss_mine
                outputs = outputs_dict
            if self.opts.loss_type == "BCE":
                # check_object_type(object=outputs, check_type=dict, name='outputs')
                loss_mine = F.binary_cross_entropy_with_logits(outputs, batch["label"].to(self.device).float())
                outputs_dict = {}
                outputs_dict['logits'] = outputs
                outputs_dict['loss'] = loss_mine
                outputs = outputs_dict
            if self.opts.loss_type == 'cross':
                ...
                # print(outputs.items())
                # print(batch['label'])
                loss_mine = CELoss(outputs["logits"], batch["label"].to(self.device))
                outputs_dict = {}
    ...
    def predict_forward(self, batch):
        ...
        with torch.no_grad():
            outputs = self.model(**inputs)
        if self.opts.loss_type is not None and "label" in batch.keys():
            if self.opts.loss_type == 'MutiChoice':
                loss_mine = multilabel_crossentropy(outputs, batch["label"].to(self.device))
                outputs_dict = {}
                outputs_dict['logits'] = outputs
                outputs_dict['loss'] = loss_mine
                outputs_dict["label"] = batch["label"]
                outputs = outputs_dict
            if self.opts.loss_type == "BCE":
                loss_mine = F.binary_cross_entropy_with_logits(outputs, batch["label"].to(self.device).float())
                outputs_dict = {}
            ...
            if self.opts.loss_type == 'cross':
                ...
                outputs_dict["label"] = batch["label"]
                outputs = outputs_dict
        elif self.opts.loss_type is not None and "label" not in batch.keys():
            if self.opts.loss_type == 'MutiChoice':
                outputs_dict = {}
                outputs_dict['logits'] = outputs
                outputs = outputs_dict
        ...
        outputs = {key: value.detach().cpu() if isinstance(value, torch.Tensor) else value for key, value in
                   outputs.items()}
        inputs = {key: value.detach().cpu() if isinstance(value, torch.Tensor) else value for key, value in
                   inputs.items()}
        batch = {key: value for key, value in dict(inputs, **outputs).items() if

...
class HeteroGNN_with_RoBERTa(torch.nn.Module):
    def __init__(self, model_name_or_path, config, tokenizer, device):
        ...
        self.device = device
        # roberta部分
        self.encoder = NeZhaForSequenceClassification.from_pretrained(model_name_or_path)
        ...
        # # 融合部分
        # self.mlp_hidden_size = 768
        # self.MLP_1 = nn.Linear(config.hidden_size + self.out_channels, self.mlp_hidden_size, bias=True)
        # self.MLP_2 = nn.Linear(self.mlp_hidden_size, config.num_labels, bias=True)
        # nn.init.normal_(self.MLP_1.weight, std=0.01)
        # nn.init.normal_(self.MLP_1.bias, std=0.01)
        # nn.init.normal_(self.MLP_2.weight, std=0.01)
        # nn.init.normal_(self.MLP_2.bias, std=0.01)
        # self.act = nn.LeakyReLU()
        # self.MLP = nn.Linear(config.hidden_size + self.out_channels, config.num_labels, bias=True)
        # nn.init.normal_(self.MLP.weight, std=0.01)
        # nn.init.normal_(self.MLP.bias, std=0.01)
    ...
    def forward(self, label, dict_, old_dict, edge_index_dict, batch):
        ...
        bert_output = \
        self.encoder(inputs['input_ids'].to(self.device), inputs['attention_mask'].to(self.device))[1][-1][:, 0, :]
        ...
        #                             truncation=True)
        #     bert_output = \
        #         self.encoder(inputs['input_ids'].to(self.device), inputs['attention_mask'].to(self.device),
        ...
        #     final_tensor = torch.FloatTensor([0.0] * 768).to(self.device)
        ...
        # bert_output = torch.Tensor(bert_output_final).to(self.device)
        ...
        #                 word = '，'
        #             new_tensor += np.array(self.embedding(torch.LongTensor([self.word2id[word]]).to(self.device)).detach().cpu()).squeeze()
        #
        ...
        new_dict = {'entities': torch.tensor(np.array(key_tensor), dtype=torch.float).to(self.device),
                    ...
                    # 'title': torch.tensor(title_tensor, dtype=torch.float).to(self.device),
                    # 'url': torch.tensor(url_tensor, dtype=torch.float).to(self.device),
                    # 'pub_time': torch.tensor(pub_time_tensor, dtype=torch.float).to(self.device),
        ...
        # hidden_tensor = torch.cat([bert_output, old_dict["father"]], dim=1)
        logits = self.MLP_2(self.act(self.MLP_1(hidden_tensor)))
        # logits = self.MLP(hidden_tensor)
        ...
        if torch.isnan(loss):
            if torch.isnan(cross_entropy(input=logits, target=label)):
                ...
                # print(logits_difference_input.reshape)
                # print(logits_difference_input)
